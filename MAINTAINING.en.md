...
| Plugin `waypoint`                      | `marketplace.json`, `plugin.json`, the plugin table, commands, and version badge URL in the READMEs, examples in `CONTRIBUTING.md`, examples in the issue forms, the banner source, the subagent and skill names in the `project-navigator` guard (`runtime/lib/guard.mjs`), the skill command in its notices (`runtime/lib/session-notices.mjs`) |
