...
- **`.navigator/` 除草稿外全部入库, init 检查项目的忽略规则.** 入库的 hook 指向 `.navigator/bin/` 下的脚本, 对账依赖入库的状态文件. 项目模板中的 `lib/`, `bin/` 之类规则会漏掉运行脚本 (实例: Python 模板的 `lib/` 让 `.navigator/bin/runtime/lib/` 从未入库). init 写入的 `.navigator/.gitignore` 重新纳入各级目录与插件写的文件类型 (`.md`, `.mjs`, `.json`, `.gitignore`), 系统与编辑器的杂项文件仍按项目规则处理, 最后排除 `drafts/`. 写好后用 `git check-ignore --no-index` 检查必须入库的路径 (运行脚本副本, 固定文件, 规格中每种记录文件的示例位置); 上级规则忽略了 `.navigator/` 本身时, 下级 `.gitignore` 不起作用, init 在改动其它文件之前停下, 报告是哪条规则. 改掉的后果: 换机器或 clone 之后 hook 找不到脚本, 守卫失效, 记录也可能漏提交.
...
| init 输出 "设置结果: 未执行" 并列出忽略规则             | 项目的忽略规则忽略了 `.navigator/` 本身, 或其中必须入库的文件                   | `commands/init.mjs`, `lib/tracked-paths.mjs`  | `git check-ignore -v --no-index <路径>`                           |
...
| `cli.test.mjs`, `flow.test.mjs` | 用项目内的命令行与 hook 走完初始化 (含忽略规则检查), 工单往返, 验收结论把关, 异常处理与体检 |
