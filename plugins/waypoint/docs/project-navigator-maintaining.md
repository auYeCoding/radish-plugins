...
7. 三种提交方式都经过 `commit-message`, 提交后对账为一致; 纳入范围之外的改动用文字询问.
8. 提交步骤中用 `-m` 被拒时, 拒绝理由列出放行的写法, 编排会话改用 heredoc 或 PowerShell 管道完成提交, 不转交用户.
9. 回执 "取证记录" 用到未登记的 MCP 工具时回复 "测试授权"; 选 A 后 `tools set` 登记, 验收子代理能调用该工具, 调用未登记的 MCP 工具被拒.
10. "测试授权" 选 B: 用户粘贴输出后写入 `user-tests.md` 成功, 改写输出被拒; 验收子代理按输出判定.
