...
- **Everything under `.navigator/` except drafts is committed, and `init` checks the project's ignore rules.** The committed hooks point to scripts under `.navigator/bin/`, and reconciliation relies on the committed state file. Rules such as `lib/` and `bin/` in project templates can leave the runtime scripts out (a real case: the `lib/` rule of the Python template kept `.navigator/bin/runtime/lib/` out of the repository). The `.navigator/.gitignore` written by `init` re-includes every directory level and the file types the plugin writes (`.md`, `.mjs`, `.json`, `.gitignore`), leaves system and editor clutter to the project's rules, and excludes `drafts/` last. It then runs `git check-ignore --no-index` on every path that must be committed (the runtime copy, the fixed files, and a sample location for each record type in the spec). When a parent rule ignores `.navigator/` itself, no nested `.gitignore` can help, so `init` stops before changing anything else and reports the rule. If changed: after a clone or on another machine the hooks cannot find their scripts, the guard stops working, and records may be left uncommitted.
...
| `init` prints "设置结果: 未执行" (setup not run) and lists ignore rules | The project's ignore rules ignore `.navigator/` itself, or files in it that must be committed                                                        | `commands/init.mjs`, `lib/tracked-paths.mjs`  | `git check-ignore -v --no-index <path>`                                                       |
...
| `cli.test.mjs`, `flow.test.mjs` | Initialization (including the ignore rule check), the work order round trip, verdict enforcement, anomaly handling, and checkups through the project's commands and hook |
