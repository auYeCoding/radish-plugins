...
4. 每条回复之前, 运行 `reply <编号> --option <组号>` 取得骨架, 在骨架上填写. "当前进展" 各行原样保留, 标题, 键名, 选项块与总结块都不改. 人类总结的正文不超过 80 字, 只写结论与用户要做的选择. 回复结束时 hook 校验版式与写作规则, 不合格会被打回一次.
...
| `reply <编号> [--option <组号>]`                         | 取得回复骨架; 不带编号时列出全部类型与编号                                                                 |
